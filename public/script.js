...
const ICONS = {
  sun: `<svg class="weather-icon sun-icon" viewBox="0 0 100 100" fill="none">
    ...
    <g class="sun-rays" stroke-width="3.5" stroke-linecap="round">
      ...
      <line x1="20" y1="80" x2="27" y2="73"/><line x1="73" y1="27" x2="80" y2="20"/>
    </g></svg>`,
  moon: `<svg class="weather-icon moon-icon" viewBox="0 0 100 100" fill="none">
    ...
    <circle class="moon-crater" cx="78" cy="58" r="2.5"/></svg>`,
  cloudDay: `<svg class="weather-icon cloud-icon" viewBox="0 0 100 100" fill="none">
    <circle cx="32" cy="32" r="14" fill="#fde047" opacity="0.95"/>
    <path class="cloud-shape" d="M28 60 Q 18 60 18 70 Q 18 80 30 80 L 72 80 Q 84 80 84 70 Q 84 60 72 58 Q 70 46 56 46 Q 42 46 40 58 Q 32 58 28 60 Z"/></svg>`,
  cloudNight: `<svg class="weather-icon cloud-icon" viewBox="0 0 100 100" fill="none">
    <path d="M40 28 A 14 14 0 1 0 30 40 A 11 11 0 0 1 40 28 Z" fill="#e2e8f0" opacity="0.9"/>
    <path class="cloud-shape" d="M28 60 Q 18 60 18 70 Q 18 80 30 80 L 72 80 Q 84 80 84 70 Q 84 60 72 58 Q 70 46 56 46 Q 42 46 40 58 Q 32 58 28 60 Z"/></svg>`,
  rain: `<svg class="weather-icon rain-icon" viewBox="0 0 100 100" fill="none">
    ...
    <ellipse class="rain-drop" cx="41" cy="85" rx="2.5" ry="5"/></svg>`,
  storm: `<svg class="weather-icon storm-icon" viewBox="0 0 100 100" fill="none">
    ...
    <path class="bolt" d="M52 60 L 38 80 L 48 80 L 42 95 L 60 72 L 50 72 L 56 60 Z"/></svg>`,
  fog: `<svg class="weather-icon fog-icon" viewBox="0 0 100 100" fill="none" stroke-linecap="round" stroke-width="5">
    ...
    <line class="fog-line" x1="25" y1="80" x2="75" y2="80"/></svg>`
};
...
function pickWeatherStyle(condicao) {
  ...
  const day = isDaytime();
  let icon, cls;
  if (cat === "storm") { icon = ICONS.storm; cls = "is-day-storm"; }
  else if (cat === "rain") { icon = ICONS.rain; cls = day ? "is-day-rain" : "is-night-rain"; }
  else if (cat === "fog") { icon = ICONS.fog; cls = day ? "is-day-cloudy" : "is-night-cloudy"; }
  else if (cat === "cloudy") { icon = day ? ICONS.cloudDay : ICONS.cloudNight; cls = day ? "is-day-cloudy" : "is-night-cloudy"; }
  else { icon = day ? ICONS.sun : ICONS.moon; cls = day ? "is-day-clear" : "is-night-clear"; }
  return { icon, cls };
}
...
document.querySelectorAll(".chip[data-cidade]").forEach(chip => {
  chip.addEventListener("click", () => { $("inputCidade").value = chip.dataset.cidade; buscarClima(); });
});

async function buscarClima() {
  const cidade = $("inputCidade").value.trim();
  const out = $("climaResult");
  if (!cidade) { out.innerHTML = erroHTML({ codigo: "ENTRADA_VAZIA", mensagem: "Digite o nome de uma cidade." }); return; }
  out.innerHTML = loadingHTML("Buscando dados climáticos...");
  try {
    const res = await fetch(API + "/clima/" + encodeURIComponent(cidade));
    const data = await res.json();
    incrementCounter();
    if (!res.ok) { out.innerHTML = erroHTML(data) + jsonViewer(data); return; }
    const coords = (data.latitude && data.longitude) ? `${data.latitude.toFixed(3)}, ${data.longitude.toFixed(3)}` : "";
    const unidade = (data.clima.unidades && data.clima.unidades.temperatura) || "C";
    const { icon, cls } = pickWeatherStyle(data.clima.condicao);
    out.innerHTML = `
...
document.querySelectorAll(".chip[data-uf]").forEach(chip => {
  chip.addEventListener("click", () => { $("inputUF").value = chip.dataset.uf; listarCidades(); });
});
...
async function listarCidades() {
  ...
  if (!uf) { out.innerHTML = erroHTML({ codigo: "ENTRADA_VAZIA", mensagem: "Digite a sigla do estado." }); return; }
